...
const CreatePlaces = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        'http://localhost:8080/api/v1/order-places',
        formData,
        {
          headers: {
  ...
  return (
    <div className="max-w-md mx-auto p-6 bg-white rounded-lg shadow-md mt-6">
      <h2 className="text-2xl font-bold mb-6 text-center text-gray-800">Create New Order Place</h2>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="flex justify-between pt-4">
          ...
          >
            {isLoading ? 'Creating...' : 'Create Order Place'}
          </button>
        </div>
      </form>
    </div>
